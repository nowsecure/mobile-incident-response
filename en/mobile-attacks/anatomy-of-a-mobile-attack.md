...
[*Include few more paragraphs as something introductory*]

This section with explore the mobile attack surface. As illustrated below, a mobile attack can involve the device later, the network later, the data center, or a combination of these. INherent platform vulnerabilities and social engineering continue to pose major opportunities for cyber thieves and thus significant challenges for those looking protect user data.

![Anatomy of a Mobile Attack](../assets/anatomy-of-a-mobile-attack.png)


#### Attack Vectors
There are four points in the mobile technology chain where malicious parties may exploit vulnerabilities to launch malicious attacks:
...
**Note** *Include few more paragrpahs as introduction*
...
#### The Data Center
Attackers targeting the data center use these main points of entry:
**Note** *Include few more paragrpahs as introduction*
...
* **Weak Input Validations** -- Many Web services overly trust the input coming from mobile applications, relying on the application to validate data provided by the end user. However, attackers can forge their own communication to the web server or bypass the application’s logic checks entirely, allowing them to take advantage of missing validation logic on the server to perform unauthorized actions.

* **Brute-Force Attaks** -- A brute-force attack simply tries to guess the valid inputs to a field, often using a high rate of attempts and dictionaries of possible values. The most common usage of a brute-force attack is on authentication, but it can also be used to discover other valid values in a Web app.
